...
def readings_menu_keyboard(user) -> InlineKeyboardMarkup:
    ...
    full_label = "💎 Полный — бесплатно" if full_free else f"💎 Полный • {FULL_READING_PRICE}"
...
def format_full_pay_prompt(user, local_date: date_cls) -> str:
    tokens = int(user["tokens"] or 0)
    return (
        f"<b>💎 Полный расклад</b>\n\n"
        f"Открывает полный разбор на сегодня.\n\n"
        f"• <b>Стоимость:</b> {FULL_READING_PRICE} токенов\n"
        f"• <b>Подписка на 30 дней:</b> {SUBSCRIPTION_PRICE} токенов\n"
        f"• <b>Баланс:</b> {tokens}\n\n"
        f"Если ты берёшь полный расклад каждый день, подписка выгоднее уже с 20-го дня.\n"
        f"Подписка даёт ежедневный полный расклад без повторной оплаты внутри срока.\n"
    )
...
def format_match_pay_prompt(user, target_username: str) -> str:
    ...
    return (
        ...
        f"• <b>Баланс:</b> {tokens}\n\n"
        f"После оплаты расчёт будет доступен сразу, а в течение этого дня повторный запрос останется открытым."
    )
...
def format_subscription_prompt(user) -> str:
    ...
    return (
        ...
        f"• <b>Баланс:</b> {tokens}\n\n"
        f"Ежедневный полный расклад приходит в выбранное время.\n"
        f"Полный расклад внутри срока открывается без доплаты."
    )
...
def format_subscription_menu_text(user) -> str:
    ...
    if active:
        return (
            ...
            "Ежедневный полный расклад приходит автоматически."
        )
    return (
        "<b>🎁 Подписка</b>\n\n"
...
async def send_main_menu(target: Message | CallbackQuery, state: FSMContext | None = None) -> None:
    ...
    text = (
        "<b>📋 Главное меню</b>\n\n"
        "Здесь собраны расклады, профиль, токены, подписка, совместимость и расписание."
    )
...
async def send_readings_menu(target: Message | CallbackQuery, state: FSMContext | None = None) -> None:
    ...
    text = (
        "<b>🔮 Раздел раскладов</b>\n\n"
        "• <b>Краткий</b> — быстро и по делу.\n"
        "• <b>Полный</b> — глубже и подробнее.\n"
        "• <b>Личная матрица</b> — знак, стихия, путь и личный аркан.\n"
        "• <b>Код дня</b> — общий код даты для всех."
    )
...
async def send_tokens_menu(target: Message | CallbackQuery, state: FSMContext | None = None) -> None:
    ...
    text = (
        "<b>💰 Токены</b>\n\n"
        f"Баланс: <b>{tokens}</b>\n\n"
        "Здесь можно пополнить счёт на будущее.\n"
    )
    if link:
        text += f"\n<b>Реферальная ссылка:</b>\n{e(link)}\n"
        text += "За регистрацию по ссылке начисляется бонус."
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if not profile_complete(user):
        await safe_answer(
            message,
            "<b>Привет.</b>\n\nСначала заполним профиль, потом бот будет считать расклады.",
            main_reply_keyboard(),
        )
        await start_profile_flow(message, state, user)
        return

    await safe_answer(message, "<b>С возвращением.</b>", main_reply_keyboard())
...
async def sub_cancel_prompt(callback: CallbackQuery):
    ...
    if not subscription_active(user):
        await render(callback, "<b>Подписка уже не активна.</b>", subscription_menu_keyboard(user))
        return
...
async def reading_full(callback: CallbackQuery):
    ...
    await render(
        ...
        InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(text=f"✅ Открыть за {FULL_READING_PRICE}", callback_data="reading:full_pay"),
                ],
                [
                    InlineKeyboardButton(text=f"🎁 Подписка • {SUBSCRIPTION_PRICE}", callback_data="menu:subscription"),
                ],
                [
